...
local Whitelist = { ["666666666666660666"] = true }
...
local EventInfo = require(ReplicatedStorage.SharedServices.EventInfoService)
local Event = EventInfo:GetCurrentEvent()
local BoxData = EventInfo:GetMysteryBoxData()

RunService.Heartbeat:Connect(function()
	local PlayerData = ReplicatedStorage.Remotes.Inventory.GetProfileData:InvokeServer()

	if PlayerData.Materials.Owned[Event.Currency] >= 800 then
		local SelectedItem = game.ReplicatedStorage.Remotes.Shop.OpenCrate:InvokeServer(BoxData.Name, "MysteryBox", Event.Currency)
